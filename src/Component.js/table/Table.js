...
import { RemoveOneProduct, addToCart, removeFromCart } from "../../Rdux/Cart/Actions";
      ...
      <tbody>
        ...
        {data.map((item, index) => (
          <tr key={index}>
            <td>{item.title}</td>
            <td>
  <img style={{ width: "70px" }} src={item.images[0]} alt="Product" />
</td>
            ...
            <td>$
                {myObject[item.id]*item.price}
            </td>
            <td>
                <MyButton color="primary" back="primary" onClick={()=>addProduct(item.id)} content="+"/>
                <MyButton  content="|"/>
                <MyButton color="danger" back="primary" onClick={()=>RemoveProduct(item.id)} content="-"/>
